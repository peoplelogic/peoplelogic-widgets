...
export const setSparkLine = (width, height, strokeWidth, percentages) => {
  ...
  percentages.map((p, i) =>
    points.push({
      xpoint: (width / numOfPoints) * 0.5 * i * 2,
      ypoint: 100 - p,
    })
  );
...
const setMask = (defs, points) => {
  let mask = defs.append("mask").attr("id", "mask");
...
const setFillBackgroundGradient = (defs, points) => {
  ...
  for (let index = 0; index < points.length - 1; index++) {
    ...
    if (
      (currentPoint < 50 && nextPoint < 50) ||
      (currentPoint > 50 && nextPoint > 50)
    ) {
      gradientGenerator(
        gradient,
        `${(100 / points.length) * (index + 1)}%`,
        getcolor(currentPoint),
        1
      );
    }
  }
  ...
  gradientGenerator(
    gradient,
    "100%",
    getcolor(points[points.length - 1].ypoint),
    1
  );
};
